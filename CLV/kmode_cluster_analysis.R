kmode_cluster_table <- function(input, output, session, data){
  df <- subset(data,
  ...
  names <- paste("Cluster ", 1:input$kMode_cluster_num, sep="")
  proportions <- c()
  df['cluster'] <- km$cluster
  for(x in 1:input$kMode_cluster_num) {
    cluster_subset <- 
  ...
  table_frame = data.frame(matrix(NA, nrow = 0, ncol = input$kMode_cluster_num))
