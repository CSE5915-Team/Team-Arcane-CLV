...
##########  Logic  ###########
bad_data_file <- readline(prompt="Enter file path of raw data: ")
clean_data_file <- readline(prompt="Enter file path of clean data: ")

# Clean data file name
#clean_data_file: C:\Users\den10\OneDrive\Documents\CSE5915\Team-Arcane-CLV\Cleandata.csv

# Bad data file name
#bad_data_file: C:\Users\den10\OneDrive\Documents\CSE5915\Team-Arcane-CLV\Rawdata.csv

# Preprocess data
bad_data <- read.csv(bad_data_file, stringsAsFactors=FALSE) # Bad data

# remove unused columns
bad_data = subset(bad_data, select = -c(prev_dprt_dt,gst_jnr,gst_chld,gst_ifnt, trip_ids))
...
customer_id <- 1:length(newid_trips)
...
min_dates = bad_data %>% group_by(customer_id) %>% summarise(min_arvl_dt = min(arvl_dt))
max_dates = bad_data %>% group_by(customer_id) %>% summarise(max_arvl_dt = max(arvl_dt))
...
bad_data$customer_lifetime <- difftime(bad_data$max_arvl_dt, bad_data$min_arvl_dt)
head(bad_data)
clean_data = bad_data
